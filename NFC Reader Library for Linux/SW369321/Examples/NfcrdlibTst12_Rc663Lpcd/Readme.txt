...
==================
  ...
  NfcrdlibTst12_Rc663Lpcd test application as well as it's known problems and
  restrictions.
...
============================================
  ...
  - NfcrdlibTst12_Rc663Lpcd can be run with CLEV663B.  (See Section-12 for
    supported versions)
  - This test application will execute the test case as choosen by the user
    which will set the Digital Filter, Charge Pump and Detection Option as per
    the test case.
  - This test application 7 Functional Scenarios. 3 out of these scenarios are
    reused for Timing Test Cases.
  - For Scenario 1 to 6, the Calibration Process is performed without any load
    on the antenna. And the presence of any card in the proximity is detected
    by the Lpcd Loop.
  - For Scenario 1 to 6, if any load (Cards : Mifare Desfire EV1 (Type A),
    ezlink Type B, Type F(Felica) RCS888, ICode SliX (SLI 15693), Stackit ICode
    ILT (18000p3m3); Phone or any metal) are brought in proximity then the test
    application detect the load the exits Lpcd loop.
  - For Scenario 7, the calibration process is performed with the load(Cards :
    Mifare Desfire EV1 (Type A), ezlink Type B, Type F(Felica) RCS888, ICode
    SliX (SLI 15693), Stackit ICode ILT (18000p3m3); Phone or any metal) on any
    antenna. When the load is removed during the Lpcd Loop, then this test
    application detect the unloading on the antenna and exits Lpcd loop.


3 Restrictions on NfcrdlibTst12_Rc663Lpcd
=========================================

  - NfcrdlibTst12_Rc663Lpcd is restricted to be run on NFC Reader Boards/ICs
...
===========================================
  ...
  - The test application is written to perform lpcd with the configuration as
    provided by the user.
  - After the LPCD loop returns success, the test application wait for the new
    input from user.
...
==================
  ...
  - cr_startup_lpc175x_6x.c
    + Startup required to compile test application LPC1769 Micro Controller.
...
========================
  ...
  - The Software can be compiled for Cortex M3 LPC1769 micro-controller from
    NXP.
  - Since this test application be configured to run on various MCU ICs and
    various NFC ICs, appropriate changes are required as mentioned in Section-.


9 Steps to build NfcrdlibTst12_Rc663Lpcd for LPC1769 with RC663 using MCUXpresso
================================================================================

  See "Importing provided SW example projects" in AN11022 (See Section-13,
  Bullet-1 below)
...
====================================================
  ...
  1) To select the board go to "Project Properties" --> "C/C++ Build" -->
     ...
     e.g. For using LPC1769 with RC663 use PHDRIVER_LPC1769RC663_BOARD.  For
...
==================================
  ...
  the NFC cards or phone in proximity, the test application will detect and
