...
phStatus_t phalMfdfLight_Int_ComputeErrorResponse(void * pDataParams, uint16_t wStatus)
{
    ...
    switch (wStatus)
    {
    case PHAL_MFDFLIGHT_RESP_OPERATION_OK:
    case PHAL_MFDFLIGHT_ISO7816_SUCCESS:
    case PHAL_MFDFLIGHT_RESP_OK:
        status = PH_ERR_SUCCESS;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_CMD_INVALID:
        status = PHAL_MFDFLIGHT_ERR_CMD_INVALID;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_FORMAT:
        status = PHAL_MFDFLIGHT_ERR_FORMAT;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_OUT_OF_EEPROM_ERROR:
        status = PHAL_MFDFLIGHT_ERR_OUT_OF_EEPROM_ERROR;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_INTEGRITY_ERROR:
        status = PHAL_MFDFLIGHT_ERR_PICC_CRYPTO;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_PARAMETER_ERROR:
        status = PHAL_MFDFLIGHT_ERR_PARAMETER_ERROR;
        break;

    case PHAL_MFDFLIGHT_RESP_NO_SUCH_KEY:
        status = PHAL_MFDFLIGHT_ERR_NO_SUCH_KEY;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_LENGTH_ERROR:
        status = PH_ERR_LENGTH_ERROR;
        break;

    case PHAL_MFDFLIGHT_RESP_PERMISSION_DENIED:
        status = PHAL_MFDFLIGHT_ERR_PERMISSION_DENIED;
        break;

    case PHAL_MFDFLIGHT_RESP_APPLICATION_NOT_FOUND:
        status = PHAL_MFDFLIGHT_ERR_APPLICATION_NOT_FOUND;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_AUTHENTICATION_ERROR:
        status = PH_ERR_AUTH_ERROR;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_AUTHENTICATION_DELAY:
        status = PH_ERR_AUTH_DELAY;
        break;

    case PHAL_MFDFLIGHT_RESP_ADDITIONAL_FRAME:
        status = PH_ERR_SUCCESS_CHAINING;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_BOUNDARY_ERROR:
        status = PHAL_MFDFLIGHT_ERR_BOUNDARY_ERROR;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_COMMAND_ABORTED:
        status = PHAL_MFDFLIGHT_ERR_COMMAND_ABORTED;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_COUNT:
        status = PHAL_MFDFLIGHT_ERR_COUNT;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_DUPLICATE:
        status = PHAL_MFDFLIGHT_ERR_DUPLICATE;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_FILE_NOT_FOUND:
        status = PHAL_MFDFLIGHT_ERR_FILE_NOT_FOUND;
        break;

    case PHAL_MFDFLIGHT_RESP_ERR_APPL_INTEGRITY_ERROR:
    case PHAL_MFDFLIGHT_RESP_ERR_PICC_INTEGRITY:
    case PHAL_MFDFLIGHT_RESP_ERR_EEPROM:
    case PHAL_MFDFLIGHT_RESP_ERR_FILE_INTEGRITY:
    case PHAL_MFDFLIGHT_RESP_ERR_PIC_DISABLED:
    case PHAL_MFDFLIGHT_RESP_ILLEGAL_COMMAND_CODE:
        status = PHAL_MFDFLIGHT_ERR_DF_GEN_ERROR;
        PH_CHECK_SUCCESS_FCT(statusTmp, phalMfdfLight_SetConfig(
            pDataParams,
            PHAL_MFDFLIGHT_ADDITIONAL_INFO,
            wStatus));
        break;

    case PHAL_MFDFLIGHT_ISO7816_ERR_WRONG_LENGTH:
    case PHAL_MFDFLIGHT_ISO7816_ERR_WRONG_LE:
    case PHAL_MFDFLIGHT_ISO7816_ERR_FILE_NOT_FOUND:
    case PHAL_MFDFLIGHT_ISO7816_ERR_WRONG_PARAMS:
    case PHAL_MFDFLIGHT_ISO7816_ERR_WRONG_LC:
    case PHAL_MFDFLIGHT_ISO7816_ERR_NO_PRECISE_DIAGNOSTICS:
    case PHAL_MFDFLIGHT_ISO7816_ERR_EOF_REACHED:
    case PHAL_MFDFLIGHT_ISO7816_ERR_LIMITED_FUNCTIONALITY_INS:
    case PHAL_MFDFLIGHT_ISO7816_ERR_FILE_ACCESS:
    case PHAL_MFDFLIGHT_ISO7816_ERR_FILE_EMPTY:
    case PHAL_MFDFLIGHT_ISO7816_ERR_MEMORY_FAILURE:
    case PHAL_MFDFLIGHT_ISO7816_ERR_INCORRECT_PARAMS:
    case PHAL_MFDFLIGHT_ISO7816_ERR_WRONG_CLA:
    case PHAL_MFDFLIGHT_ISO7816_ERR_UNSUPPORTED_INS:

        status = PHAL_MFDFLIGHT_ERR_DF_7816_GEN_ERROR;
        PH_CHECK_SUCCESS_FCT(statusTmp, phalMfdfLight_SetConfig(
            pDataParams,
            PHAL_MFDFLIGHT_ADDITIONAL_INFO,
            wStatus));
    ...
    case PHAL_MFDFLIGHT_RESP_CHAINING:
        status = PH_ERR_SUCCESS_CHAINING;
        break;
 ...
 */
phStatus_t phalMfdfLight_ExchangeCmd(void * pDataParams, void * pPalMifareDataParams, uint8_t bWrappedMode, uint8_t * pSendBuff, uint16_t wCmdLen,
...
phStatus_t phalMfdfLight_Int_Send7816Apdu(void * pDataParams, void * pPalMifareDataParams, uint8_t bOption, uint8_t bIns, uint8_t bP1, uint8_t bP2,
    uint32_t bLc, uint8_t bExtendedLenApdu, uint8_t * pDataIn, uint32_t bLe, uint8_t ** ppDataOut, uint16_t *pDataLen)
{
    ...
    switch (bIns)
    {
    case PHAL_MFDFLIGHT_CMD_ISO7816_SELECT_FILE:
        /* Both Lc and Le are required */
        statusTmp = phalMfdfLight_Int_ISOSelectFile(pDataParams, pPalMifareDataParams, bOption, bCmdBuff, wCmdLen,
            (uint8_t)bLc, bExtendedLenApdu, pDataIn, (uint8_t)bLe, ppDataOut, pDataLen);
        break;

    case PHAL_MFDFLIGHT_CMD_ISO7816_UPDATE_BINARY:
        statusTmp = phalMfdfLight_Int_ISOUpdateBinary(pDataParams, pPalMifareDataParams, bOption, bCmdBuff, wCmdLen,
            bLc, bExtendedLenApdu, pDataIn, ppDataOut, pDataLen);
        break;

    default:
        return PH_ADD_COMPCODE_FIXED(PH_ERR_UNSUPPORTED_COMMAND, PH_COMP_AL_MFDFLIGHT);
        break;
    }
    return statusTmp;
}
    ...
    uint8_t bLc, uint8_t bExtendedLenApdu, uint8_t * pDataIn, uint8_t bLe, uint8_t ** ppDataOut, uint16_t *pDataLen)
{
    ...
    */
    if ((bOption & 0x02) && (bCmdBuff[2] != 0x03))
    {
    ...
    {
        PH_CHECK_SUCCESS_FCT(status, phalMfdfLight_GetConfig(pDataParams, PHAL_MFDFLIGHT_ADDITIONAL_INFO, &wVal));
    }
    ...
    uint32_t dwLc, uint8_t bExtendedLenApdu, uint8_t * pDataIn, uint8_t ** ppDataOut, uint16_t *pDataLen)
{
    ...
    statusTmp <<= 8;
    statusTmp |= pRecv[wRxlen - 1];
    PH_CHECK_SUCCESS_FCT(statusTmp, phalMfdfLight_Int_ComputeErrorResponse(pDataParams, statusTmp));
    if (pDataLen != NULL)
    {
        *pDataLen = wRxlen - 2;
    }
