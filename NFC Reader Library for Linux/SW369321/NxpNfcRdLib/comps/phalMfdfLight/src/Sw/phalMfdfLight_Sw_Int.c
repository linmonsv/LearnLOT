...
/** \file
* Internal source file used by software MIFARE DESFire Light Application Component
...
/* Validate parameters
   ...
   /* Validate Card Key Number */
    ...
    if ((wOption != PHAL_MFDFLIGHT_NO_DIVERSIFICATION) && (bDivLen > 31))
    {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_AL_MFDFLIGHT);
    }
    if(bLenPcdCapsIn > 6)
    ...
    if (bAuthOption == PHAL_MFDFLIGHT_AUTHFIRST_NON_LRP)
    {
        ...
        memcpy(pDataParams->bTi, &bCmdBuff[0], PHAL_MFDFLIGHT_SIZE_TI);  /* PRQA S 3200 */ /* Store the Transaction Identifier */
    ...
    uint8_t bCommOption, uint8_t * pData, uint16_t wDataLen)
{
    ...
    if (pDataParams->bAuthMode == PHAL_MFDFLIGHT_AUTHENTICATEEV2)
    {
        ...
        if (wDataLen1)
        {
            if((wTmp < 32) && ((bCmdBuff[0] != PHAL_MFDFLIGHT_RESP_ADDITIONAL_FRAME)))
            {
                wCommMode = PH_EXCHANGE_BUFFER_FIRST;
            }
    ...
    */
    ...
    {
        wApduLen = ((wCmdLen == 0x01 ) && (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE ))? 0 : PHAL_MFDFLIGHT_WRAP_HDR_LEN;
        wCmdLen = ((wCmdLen == 0x01 ) && (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE ))? 0 : wCmdLen;
       ...
       pApdu[1] = bCmdBuff[0]; /* Desfire cmd code in INS */
        ...
        PH_CHECK_SUCCESS_FCT(statusTmp, phpalMifare_ExchangeL4(
            ...
            (((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE) && (bCommOption & PHAL_MFDFLIGHT_MAC_DATA_INCOMPLETE)) ? PH_EXCHANGE_TXCHAINING : 0),
            &pApdu[2],
            (uint16_t)((pDataParams->dwPayLoadLen > 0xFE ) && (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE ))? 0x02U : 0x01U,
            &pRecv,
            &wRxlen));
         ...
         */
        if((pApdu[1] == PHAL_MFDFLIGHT_CMD_SET_CONFIG) &&
    ...
    uint8_t * pData, uint16_t wDataLen, uint8_t * bLastChunk, uint16_t wLastChunkLen, uint8_t * pResp, uint16_t * pRespLen)
{
    ...
    else
    {
        ...
        wApduHeaderLen = PHAL_MFDFLIGHT_WRAP_HDR_LEN + (bExtended7816 ? 2 : 0);
    ...
    do
    {
        ...
        wLeFieldLen = ((bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE) && (pDataParams->bWrappedMode == 1)) ? 1 : 0;
        ...
        {
            bStatusByte = (uint8_t)((pRecv[0] & 0xF0) == 0xA0) ? PHAL_MFDFLIGHT_RESP_ADDITIONAL_FRAME : PH_ERR_PROTOCOL_ERROR;
        }
        else
        {
        ...
        if(bStatusByte != 0x00)
        {
            ...
            wCmdLen = (bIns == PHAL_MFDFLIGHT_ISO_CHAINING_MODE)? 0: 1;
    ...
    uint32_t * pBytesRead)
{
    ...
    if (status == PH_ERR_SUCCESS)
    {
        statusTmp = (*ppRxBuffer)[wRxlen - 2]; /* SW1 */
        statusTmp = statusTmp << 8; /* Shift SW1 to MSB */
        statusTmp |= (*ppRxBuffer)[wRxlen - 1]; /* SW2 */
...
phStatus_t phalMfdfLight_Sw_Int_buildLrpSV(uint8_t *pRndA, uint8_t *pRndB, uint8_t *pSV)
{
    ...
    uint8_t bSVpostfix[2] = { 0x96, 0x69 };
    uint8_t index = 0;

    if((pRndA ==  NULL) || (pRndB ==  NULL) || (pSV == NULL))
    {
        return PH_ADD_COMPCODE_FIXED(PH_ERR_INVALID_PARAMETER, PH_COMP_AL_MFDFLIGHT);
    }
    /* Copy the SVPrefix */
    memcpy(&pSV[0], bSVprefix, sizeof(bSVprefix));
    memcpy(&pSV[4], &pRndA[0] , 8);
    memcpy(&pSV[22], &pRndA[8], 8);
    memcpy(&pSV[12], &pRndB[6], 10);
    for ( index = 0; index < 6; index++)
    {
        pSV[index + 6] ^= pRndB[index];
    }
    memcpy(&pSV[30], bSVpostfix, sizeof(bSVpostfix));
